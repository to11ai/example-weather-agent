...
### 02 — Gateway
The *only* change from 01: point the OpenAI client `baseURL` at `${TO11_GATEWAY_URL}` (the gateway is OpenAI-compatible, so the OpenAI SDK works unchanged) and set the to11 auth headers **inline** — `x-to11-authorization: Bearer <to11 key>`, `x-to11-project-id`, `x-to11-env`. **No to11 SDK is needed to route through the gateway** (it's introduced in step 04 for prompt management). Prompt and tools unchanged. **Payoff:** full request/response telemetry with zero prompt changes; README shows the dashboard trace and the `x-to11-request-id` response header. **Env adds:** `TO11_API_KEY`, `TO11_PROJECT_ID`, `TO11_GATEWAY_URL` (default `https://gw.to11.ai/v1`), `TO11_ENV` (default `prod`). `OPENAI_API_KEY` is still set and forwarded upstream by the gateway.
